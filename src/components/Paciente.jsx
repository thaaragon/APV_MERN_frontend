...
const Paciente = ({ paciente }) => {
    ...
    const { setEdiion, eliminarPaciente } = usePacientes()

    const { email, fecha, nombre, propietario, sintomas, _id } = paciente


    const formmaterarFecha = (fecha) => {
    ...
    return (
        <div className="mx-5 my-10 bg-white shadow-md px-5 py-10 rounded-xl">
            ...
            <p className="font-bold uppercase text-indigo-700 my-2">Fecha: {''}
                <span className="font-normal normal-case text-black">{formmaterarFecha(fecha)}</span>
            </p>
